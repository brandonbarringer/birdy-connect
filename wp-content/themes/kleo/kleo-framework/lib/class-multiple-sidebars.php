...
if (! class_exists('sidebar_generator')) {
	class sidebar_generator
	{
						...
						foreach ($sidebars as $sidebar) {
							...
							<tr class="<?php echo $alt ?>">
								...
								<td><a href="javascript:void(0);"
									   onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt + 1; ?>);"
		...
		public static function get_sidebar($name = "0")
		{
			...
			if( ! empty( $selected_sidebar_replacement ) && ( isset($selected_sidebar_replacement) && $selected_sidebar_replacement[0] !== '0' ) ) {

				if ( function_exists( 'is_woocommerce' ) ) {
					if ( is_woocommerce() ) {
						$selected_sidebar = array( 'shop-1' );
					}
				}
			}
